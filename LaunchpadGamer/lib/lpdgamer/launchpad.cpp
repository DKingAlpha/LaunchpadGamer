...
LaunchpadBase::LaunchpadBase() 
{
    ...
    sendmessage_mutex = CreateMutex(NULL, NULL, NULL);
}
...
void LaunchpadPro::playMidiFile(std::string file) {
    ...
    for (int event = 0; event < midifile[0].size(); event++)
    {
        ...
        if (keytype == 0x80 || keytype == 0x90)
        {
            ...
            if(PadStatus[key] == 1)setColor(key, color);
        }
    }
}

void LaunchpadPro::loadConfig(std::string mapgame)
{
    game_to_map = mapgame;
